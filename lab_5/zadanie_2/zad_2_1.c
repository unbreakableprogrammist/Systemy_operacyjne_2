...
void child_work(int i, int P[2], int R[2]) {
    // 1. Inicjujemy generator liczb losowych (unikalny dla każdego dziecka)
    srand(getpid()); 
    
    // 2. Losujemy znak od 'a' do 'z'
    char c = 'a' + (rand() % 26);
    
    // 3. Wypisujemy losową literę do wspólnego pipe R
    if(TEMP_FAILURE_RETRY(write(R[1], &c, 1)) < 0) ERR("write R in child");
...
int main(int argc, char** argv) {
    ...
    // --- GŁÓWNA PRACA RODZICA (Etap 1) ---
    char odebrany_znak;
    ssize_t count;
    
    printf("Rodzic czeka na dane od dzieci...\n");
    
    // Czytamy w pętli. Funkcja read zwróci 0 dopiero, gdy WSZYSTKIE dzieci zamkną rurę R[1]
    while((count = TEMP_FAILURE_RETRY(read(R[0], &odebrany_znak, 1))) > 0) {
        printf("Odebrano: %c\n", odebrany_znak);
    }
    if(count < 0) ERR("read R[0] in parent");
    
    printf("Wszystkie dzieci zakonczyly przesyl. Rodzic tez konczy.\n");
